...
TNBC = clinvar[clinvar$CancerType == cancer,]
TNBC = TNBC[TNBC$her2_status_by_ihc == "Negative" & TNBC$pr_status_by_ihc == "Negative" & TNBC$er_status_by_ihc == "Negative",]


TumorCounts_subset=TumorCounts[,colnames(TumorCounts) %in% rownames(TNBC)]
...
pheatmap(scale(normalized_cds_sig), col = my.cols, annotation = annotation, cellwidth=3, cellheight=6,annotation_colors = anno_colors,
         show_colnames=F, legend=T, show_rownames=F, filename=paste("TNBC", "/dz_sig_validation_lincs_reduced_v0.pdf", sep="")
)
pheatmap(scale(normalized_cds_sig), col = my.cols, annotation = annotation, annotation_colors = anno_colors,
         show_colnames=F, legend=T, show_rownames=F, filename=paste("TNBC", "/dz_sig_validation_lincs_reduced_v1.pdf", sep="")
)
