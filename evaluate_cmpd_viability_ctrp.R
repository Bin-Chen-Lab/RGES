#compare with other drug sensitivity databases including CTRP.
#
cancer <- "COAD"
site <- "large_intestine" #breast #large_intestine #liver
...
cell_lines <- read.csv(paste("~/Documents/stanford/tumor_cell_line/RGES_manuscript/table/", cancer, "_cell_lines.csv", sep=""))
...
drug_cell_matrix_subset <-  drug_cell_matrix[,colnames(drug_cell_matrix) %in% ctrp_cell_lines_site]#cell_lines_subset$CTRP
...
print(p + stat_smooth(method="lm", se=F, color="black")  + geom_point() + 
        annotate("text", label = paste(cancer, ", ", "r=", format(summary(lm_cmap_ic50)$r.squared ^ 0.5, digit=2), sep=""), x = 0, y = 18.5, size = 6, colour = "black") +
        ...
        ylab("AUC") + coord_cartesian(xlim = c(-0.5, 0.5), ylim=c(0, 20))) 
dev.off()
