...
pdf(paste( "fig/", cancer, "rges_auc_gdsc.pdf", sep=""))
...
print(p + stat_smooth(method="lm", se=F, color="black")  + geom_point() + 
        annotate("text", label = paste(cancer, ", ", "r=", format(summary(lm_cmap_ic50)$r.squared ^ 0.5, digit=2), sep=""), x = 0, y = 9, size = 6, colour = "black") +
        annotate("text", label = paste("rho=", format(cor_test$estimate, digit=2), ", P=", format(cor_test$p.value, digit=3, scientific=T), sep=""), x = 0, y = 7.5, size = 6, colour = "black") +
        scale_size(range = c(2, 5)) +
        xlab("sRGES") + guides(shape=FALSE, size=FALSE) +
        ylab("log(IC50s)") + coord_cartesian(xlim = c(-0.5, 0.5), ylim=c(-10, 10))) 
dev.off()
